function processTrainingCenterCommands(commands) {
	...
	var trainingcenter = (function() {
		...
		var Trainer = (function() {
			var Trainer = {
				...
				toString: function() {
					...
					var mailText = this.getMail() ? ';email=' + this.getMail() : '';

					return 'Trainer[username=' + this.getUsername() + firstNameText + ';last-name=' + this.getLastName() + mailText + ']';
				},
				type: 'Trainer'
			};
		...
		var Training = (function() {
			var Training = {
				...
				toString: function() {
					...
					var trainerText = this.getTrainer() ? ';trainer=' + this.getTrainer() : '';
					var durationText = this.getDuration() ? ';duration=' + this.getDuration() : '';
					return this.type + '[name=' + this.getName() + descriptionText + trainerText + ';start-date=' + formatDate(this.getStartDate()) + durationText + ']';
				},
				type: 'Training'
			};
		...
		var Seminar = (function() {
			...
			Object.defineProperty(Seminar, 'toString', {
				value: function(){
					var result = Training.toString.call(this);
					return result.replace(';start-date=', ';date=');
				}
			});
		...
		var RemoteCourse = (function() {
			...
			Object.defineProperty(RemoteCourse, 'toString', {
				value: function() {
					var parentText = Course.toString.call(this);
					parentText = parentText.slice(0, parentText.length - 1);
					var locationText = this.getLocation() ? ';location=' + this.getLocation() : '';
					return parentText + locationText + ']';
				}
			});
